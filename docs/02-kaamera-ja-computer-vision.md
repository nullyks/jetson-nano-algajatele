...
| --- | --- | --- |
| CSI kaamera | CSI/MIPI | `/dev/video0` |
| USB kaamera | USB | `/dev/video1` |
...
Miks see vajalik on: CSI kaamera ja USB kaamera saavad töötada eri toorvormingutes. Kaamera pakutud vorming peab vastama GStreameri töövoo kapslitele, muidu tuleb läbirääkimisviga.

Oodatud tulemus: tuvastad, kas CSI kaamera on tõesti `/dev/video0`, USB kaamera `/dev/video1`, ning millist `MJPG`, `YUYV` või muud vormingut USB kaamera pakub. Täit väljundit ära avalda, sest selles võivad olla seadme seerianumber ja muud lokaalsed andmed.

## CSI kaamera: `RG10` Bayeri kaader seadmest `/dev/video0`

Selles seadistuses on CSI kaamera seotud `/dev/video0` seadmega, kuid selle V4L2 vorming on `RG10`: 10-bitine Bayeri toorandmestik. `videoconvert` ei tööta selle vorminguga otse, seega `v4l2src ! videoconvert ! jpegenc` annab vea `not-negotiated`. Kasuta CSI kaamera jaoks Arguse teed:
...
Miks see vajalik on: kaamera füüsiline ühendus ja Linuxi videoseadme liides ei ole sama asi. Arguse tee töötleb CSI kaamera Bayeri toorandmestikku õigesti ning on selle seadistusega läbi proovitud.
...
## USB kaamera: üks kaader seadmest `/dev/video1`

USB kaamera pakub selles seadistuses `MJPG` vorminguga 1920×1080 pilti 30 fps juures. Salvesta kaader nii:
...
Mida see käsurühm teeb: USB kaamera annab ühe MJPEG kaadri. `jpegparse` töötleb seda JPEG voona ja `filesink` kirjutab pildi ilma seda esmalt toorvideoks dekodeerimata.

Miks see vajalik on: MJPEG korral on kaamera kaader juba JPEG kujul ning see tee on lihtne ja väikese lisakoormusega.

Oodatud tulemus: `m9-pro-mjpg.jpg` on loetav JPEG pilt. See 1920×1080 ja 30 fps näide on selle USB kaamera seadistusega läbi proovitud. Kui kapslid ei sobi teises seadistuses, ära proovi juhuslikke väärtusi, vaid kopeeri vorminguloendist õige laius, kõrgus ja kaadrisagedus.

Kui USB kaamera pakub `YUYV` vormingut, kasuta seda varianti:
...
Oodatud tulemus: luuakse `m9-pro-yuyv.jpg`. Ka selles käsus peavad näite laius, kõrgus ja kaadrisagedus vastama USB kaamera tegelikule vormingule.
...
- V4L2 voog: sobib CSI kaamera või USB kaamera reaalajas töötluseks;
